...
router.post('/signup', async (req, res) => {
  ...
  try {
    if (!/^[a-z]+[a-z0-9]{3,16}$/g.test(nickname))
      return res.status(412).json({
        errorMessage: 'nickname값은 영문으로 시작하는 영문, 숫자 3~16자를 입력해 주세요.',
      });
    if (!/^(?=.*[a-zA-z])(?=.*[0-9])(?=.*[$`~!@$!%*#^?&\-_=+]).{4,16}$/.test(password))
      return res.status(412).json({
        errorMessage: 'password값은 4~16자 영문, 숫자, 특수문자를 최소 한가지씩 조합해서 입력해 주세요.',
      });
