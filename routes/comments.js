...
router.post("/posts/:postId/comments", authMiddleware, async (req, res) => {
  ...
  if (!comment) {
    ...
    Comments.create({ PostId: postId, UserId: userId, nickname, comment });
...
router.put("/posts/:postId/comments/:commentId", authMiddleware, async (req, res) => {
  ...
  if (!comments) {
    ...
    return res
      .status(412)
      .json({ errorMessage: "댓글의 내용을 입력해 주세요" });
  } else if (userId !== comments.UserId) {
    return res
      .status(412)
      .json({ errorMessage: "댓글 수정권한이 없습니다." });
  }
  try {
...
router.delete("/posts/:postId/comments/:commentId", authMiddleware, async (req, res) => {
  ...
  if (comment) {
    ...
    res
      .status(404)
      .json({ errorMessage: "이미 삭제되었거나 댓글이 존재하지 않습니다." });
  }
});
