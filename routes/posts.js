...
router.get('/posts', async (req, res) => {
  try {
    const posts = await Posts.findAll({
      attributes: ['postId', 'userId', 'nickname', 'title', 'likes', 'createdAt', 'updatedAt'],
...
router.get('/posts/:postId', async (req, res) => {
  ...
  try {
    const posts = await Posts.findOne({
      attributes: ['postId', 'userId', 'nickname', 'title', 'content', 'likes', 'createdAt', 'updatedAt'],
      where: { postId },
    });
    res.status(200).json({ posts });
  } catch (error) {
...
router.put('/posts/:postId', authMiddleware, async (req, res) => {
  ...
  if (!post) {
    ...
    return res.status(412).json({ errorMessage: '게시글 제목을 입력해 주세요' });
  } else if (!content) {
    return res.status(412).json({ errorMessage: '게시글 내용을 입력해 주세요' });
  ...
  try {
    await Posts.update({ title, content }, { where: { [Op.and]: [{ postId }, { userId }] } });
